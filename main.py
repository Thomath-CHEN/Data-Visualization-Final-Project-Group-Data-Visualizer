...
def Dynamic_Viewer(path):
    ...
    st.title("Welcom to Dynamic Viewer")
...
def Electricity_Trends(path):
    ...
    st.title("Welcom to Emission Trends")
...
def Electricity_Source(path):
    ...
    st.title("Welcom to Electricity Source")
    subtitle = '<p style="font-family:sans-serif; color:Black; font-size: 25px;">Electricity generation by source</p>'
    ...
    df = df_raw[['Year', 'Country Name', 'EG.ELC.FOSL.ZS', 'EG.ELC.HYRO.ZS', 'EG.ELC.NUCL.ZS', 'EG.ELC.RNWX.ZS', 'EG.USE.ELEC.KH.PC', 'SP.RUR.TOTL', 'SP.URB.TOTL']]
    ...
    df = pd.concat([df[['Year', 'Country Name']], df[['Fossil %', 'Hydro %', 'Nuclear %', 'Other Renewable Excluding Hydro %', 'Total Electricity Generation (billion KWh)']].round(2)], axis=1)
    ...
    # let user select a country from a dropdown list on Streamlit
    ...
    # display both the "percentage of electricity generation by source" and the "total electricity generation" charts
...
def Emission_Predictor(path):
    ...
    st.title("Welcom to Emission Predictor")
